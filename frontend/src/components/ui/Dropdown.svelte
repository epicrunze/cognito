...
<div bind:this={ref} style="position: relative; display: inline-flex; width: {width}px;">
  <button
    type="button"
    onclick={() => open = !open}
    style="width: 100%; height: 34px; padding: 0 12px; font-size: 13.5px; font-weight: 400; color: {selected ? 'var(--text-primary)' : 'var(--text-tertiary)'}; background: var(--bg-elevated); border: 1px solid {open ? 'var(--accent)' : 'var(--border-default)'}; border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: space-between; transition-property: background-color, border-color, color, box-shadow, transform, opacity; transition-duration: var(--t-fast); transition-timing-function: var(--ease-out); outline: none; box-shadow: {open ? '0 0 0 2px rgba(232,119,46,0.15)' : 'none'}; font-family: var(--font-sans);"
  >
