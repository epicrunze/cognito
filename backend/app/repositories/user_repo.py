"""
User repository — DuckDB CRUD for the users table.

Ported from archive/thought-journal with no functional changes.
"""
...
def get_user_by_email(conn: duckdb.DuckDBPyConnection, email: str) -> Optional[UserInDB]:
    result = conn.execute(
    ...
    return UserInDB(
        id=result[0] if isinstance(result[0], UUID) else UUID(str(result[0])),
        email=result[1],
        name=result[2],
        picture=result[3],
...
def create_user(conn: duckdb.DuckDBPyConnection, user: User) -> UserInDB:
    ...
    try:
        conn.execute(
            """INSERT INTO users (id, email, name, picture, created_at, last_login_at)
               VALUES (?, ?, ?, ?, ?, ?)""",
            [str(user_id), user.email, user.name, user.picture, now, now],
        )
        return UserInDB(
            id=user_id,
            email=user.email,
...
def update_last_login(conn: duckdb.DuckDBPyConnection, user_id: UUID) -> None:
...
def clear_refresh_token(conn: duckdb.DuckDBPyConnection, user_id: UUID) -> None:
    conn.execute(
...
def get_user_by_id(conn: duckdb.DuckDBPyConnection, user_id: UUID) -> Optional[UserInDB]:
    result = conn.execute(
        """SELECT id, email, name, picture, created_at, last_login_at,
                  refresh_token, refresh_token_expires_at
           FROM users WHERE id = ?""",
